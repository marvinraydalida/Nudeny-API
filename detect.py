...
class NudenyDetect:
    ...
    def __init__(self):
        ...
        self.category_index = {1: {'id': 1, 'name': 'buttocks'}, 2: {'id': 2, 'name': 'female_breast'}, 3: {'id': 3, 'name': 'female_genitalia'}, 4: {'id': 4, 'name': 'male_genitalia'}}
    ...
    def draw_bounding_box(self, file, filename):
        ...
        cv2.imwrite(os.path.join('./tmp', filename), image_with_detections)
        ...
        for scores in detections['detection_scores']:
            if scores >= 0.5:
                ...
                key = self.category_index[detections['detection_classes'][index]]['name']
                exposed_parts[key] = {"confidence_score": scores * 100, "top": int(top), "left": int(left), "bottom": int(bottom), "right": int(right)}
            else:
                break
            index += 1
    ...
    def censor_exposed_part(self, file, filename):
        ...
        cv2.imwrite(os.path.join('./tmp', filename), censored_image)
